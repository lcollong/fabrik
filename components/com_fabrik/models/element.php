...
class plgFabrik_Element extends FabrikPlugin
{
	...
	*/
	...
	function preRender($c, $elCount, $tmpl)
	{
		...
		{
			$element->element_raw = array_key_exists($elHTMLName, $model->_data) ? $model->_data[$elHTMLName] : $element->value;
		}
	...
	public function getElementOutPut($data, $repeatCounter = 0, &$groupModel)
	{
		if (!$this->canView() && !$this->canUse())
		{
			return '';
	...
	*/
	...
	public function getJoinModel()
	{
		if (is_null($this->joinModel))
		{
			...
			$this->joinModel->setElementId($this->getElement()->id);
		}
	...
	public function selfDiagnose()
	{
		...
		else if (!$this->element->plugin)
		{
			$retStr = 'No plugin';
		}
		else if (!$this->element->label)
		{
	...
	public function inLineEdit()
	{
		...
		if (!$saving)
		{
			...
			$html .= $this->getElementOutPut($data, $repeatCounter, $groupModel);
			$html .= '</li>';
			$html .= '</ul>';
